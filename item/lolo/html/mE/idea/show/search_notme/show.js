...
function IdeA_search(search){
	...
	$('#LoadiN_tip').css('display','block');	 	  	 
	 var xmlhttp=$.ajax({
	 	url:ROOTDIR+'html/mE/idea/show/search_notme/show.php?action=idea_search',
	 	type:'POST',
	 	data:'search='+search+'&mine='+mine+'&hid='+hid,
	 	datatype:'html',
	 	success:function(){
...
function ResulT_ideasearch(result){	
	...
	$('#LoadiN_tipcon').css('margin','5px auto 0');				
		$('#LoadiN_tipcon').css({top:0,left:0});		
			$('#LoadiN_tip').css('display','none');					$('#LoadiN_tipstr').html('');						
}
