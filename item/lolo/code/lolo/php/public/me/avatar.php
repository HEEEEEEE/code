...
if($_GET['action']=='user_avatar'){
	...
	include ROOTDIR.'include/engine/fu/fu_file.php';
		...
		$file_type=file_upfiltertype('upload-imageavatar','image/png');
		$file_fsize=file_upfiltersize('upload-imageavatar',2500000);
		$file_name=file_upfiltername('upload-imageavatar',250,'png');
		...
		$expand=explode('/',$_FILES['upload-imageavatar']['type']);	
	 ...
	 */	
		...
		if($result && $result_idea){
			//save sources start save sources start save sources start
				//file to up:move
				//Basic
				$tmp=$_FILES['upload-imageavatar']['tmp_name'];
